...
Use `@modelcontextprotocol/server` for the server and `@modelcontextprotocol/client` for clients. `@chrischall/mcp-utils` 0.28 supplies the matching bootstrap and test helpers. Tool schemas must be complete Zod objects, rather than raw shapes. Schema inspection tests read `.shape` directly.
